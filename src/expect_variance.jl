...
"""
function expect(indices, op::AbstractOperator{KetBraBasis{B1,B2}}, state::AbstractOperator{KetBraBasis{B3,B3}}) where {B1,B2,B3<:CompositeBasis}
    N = length(fullbasis(state).left.shape)
    indices_ = complement(N, indices)
    expect(op, ptrace(state, indices_))
end

expect(index::Integer, op::AbstractOperator{KetBraBasis{B1,B2}}, state::AbstractOperator{KetBraBasis{B3,B3}}) where {B1,B2,B3<:CompositeBasis} = expect([index], op, state)
...
If an `index` is given, it assumes that `op` is defined in the subsystem specified by this number
"""
function variance(indices, op::AbstractOperator{KetBraBasis{B,B}}, state::AbstractOperator{KetBraBasis{BC,BC}}) where {B,BC<:CompositeBasis}
    N = length(fullbasis(state).left.shape)
    indices_ = complement(N, indices)
    variance(op, ptrace(state, indices_))
end

variance(index::Integer, op::AbstractOperator{KetBraBasis{B,B}}, state::AbstractOperator{KetBraBasis{BC,BC}}) where {B,BC<:CompositeBasis} = variance([index], op, state)
